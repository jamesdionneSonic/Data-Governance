...
- Final verification/publish pass 94: full `npm test` passed with `41` suites and `454` tests. `npm run catalog:runtime:check` passed with `7224` objects, `29798` typed edges, and heap `189` MB. `npm run catalog:repo:check` passed with `7224` objects, `14972` generated files, warnings `[]`, failures `[]`. Direct searches found no `unknown.Sonic_DW` in active raw data, generated markdown, Confluence export text artifacts, or `C:\projects\Sonic-data-lineage`.
- Final publish evidence pass 94: pushed app hardening commit `0b29f4f` (`Quarantine unknown Sonic_DW records`) to `origin/main`. Pushed Azure DevOps lineage repo commit `4244e09d2` (`Remove quarantined unknown Sonic_DW records`) to `origin/master`. Both local repos reported clean status after push.
- Runtime package backlog pass 95: created `docs/LINEAGE_RUNTIME_PACKAGE_BACKLOG.md` to operationalize the Azure Artifacts Universal Package plan for `sonic-data-lineage-runtime`. The backlog defines the runtime contract, AI retrieval order, quality gates, local package build/check commands, and publish blockers.
- Runtime package implementation pass 95: added `npm run lineage:runtime:package`, `npm run lineage:runtime:check`, `npm run lineage:runtime:prepare`, and `npm run lineage:runtime:publish`. The package builder copies the validated DevOps catalog repo into ignored local runtime package output, writes `manifest.json`, `latest.json`, and `docs/runtime-package-guide.md`, and preserves registry/context-pack entrypoints for AI lookup.
- Runtime package validation evidence pass 95: `node --check` passed for all three new runtime package scripts. `npm run lineage:runtime:package` built `C:\projects\Data Governence\data\lineage-runtime-package\sonic-data-lineage-runtime` with package name `sonic-data-lineage-runtime`, version `2026.6.4-1`, `7224` objects, `7224` registry JSONL rows, `14975` files, and `73177075` bytes.
- Runtime package quality-gate evidence pass 95: first validation correctly failed because the generated package README/manifest documented the forbidden stale literal; after removing that literal from generated package docs/manifest, `npm run lineage:runtime:check` exited `0` with status `ok`, `7224` objects, `14975` files, longest path length `206`, failures `[]`, warnings `[]`. Direct `rg -n "unknown\.Sonic_DW" data/lineage-runtime-package/sonic-data-lineage-runtime` returned no matches.
- Runtime package publish blocker pass 95: `npm run lineage:runtime:publish -- --dry-run` reached the publish wrapper and blocked only on missing `AZURE_ARTIFACTS_FEED`. `az --version` also failed because Azure CLI is not installed or not on `PATH`; live Universal Package publish remains blocked until feed name and Azure CLI auth are available.
- Runtime package final verification pass 95: reran `npm run catalog:repo:check`; exit `0`; DevOps catalog repo status `ok`, `7224` objects, `14972` generated files, warnings `[]`, failures `[]`. Reran `npm run lineage:runtime:check`; exit `0`; runtime package status `ok`, warnings `[]`, failures `[]`. `git diff --check` passed. Full `npm test` passed with `41` suites and `454` tests.
- dbSonicDW canonicalization pass 96: traced `dbSonicDW` to SSIS XML connection metadata and SSIS-observed SQL endpoints, not to an active SQL Server database folder. The active raw SQL catalog had no `data/analysis/raw/sqlserver/servers/*/databases/dbSonicDW` source; the bad generated records were SSIS-inferred endpoint IDs such as `192.224.101.80.dbSonicDW.dbo.dwFullTextConversation`.
- Root-cause evidence pass 96: the ingestion engine already knew `dbsonicdw -> Sonic_DW`, but SSIS endpoint normalization parsed IPv4 IDs by splitting on dots, so `192.224.101.80.dbSonicDW...` could be interpreted with database `224` or emitted before canonical endpoint IDs were applied. Hardened `parseSqlId`, `ensureSsisSqlEndpoint`, and `normalizeSsisReference` so SSIS-observed endpoints and edge references canonicalize `dbSonicDW` to `Sonic_DW` while preserving server `192.224.101.80`.
- Regression evidence pass 96: added focused coverage proving a raw SSIS endpoint `192.224.101.80.dbSonicDW.dbo.dwFullTextConversation` is emitted as `192.224.101.80.Sonic_DW.dbo.dwFullTextConversation` with database `Sonic_DW`, including column IDs. Focused tests for rebuild/search passed: `2` suites, `14` tests.
- Clean rebuild evidence pass 96: reran `node scripts/rebuild-catalog-from-raw.mjs --enforce-gates --clean`; exit `0`; regenerated `7224` manifest files with `6399` SQL objects, `2058` SSIS packages, `25939` SSIS edges, `369` SSIS SQL endpoint records, `0` invalid objects, and `0` package confidence regressions. Runtime indexing/check loaded `7224` objects and `29808` typed edges; the canonical SSIS endpoint now appears under `Sonic_DW`.
- DevOps dataset evidence pass 96: reran `npm run catalog:repo:export` and `npm run catalog:repo:check`; check status `ok`, `7224` objects, `7224` manifest objects, `22196` generated files, warnings `[]`, failures `[]`. `registry/database-index.json` reported `34` databases, no `dbSonicDW` database entry, and `Sonic_DW` with `1747` objects. The only remaining `dbSonicDW` text in the DevOps repo is the legitimate `ETL_Staging.dbo.dwStatus_dbSonicDW` object name, not a database bucket.
- Runtime package evidence pass 96: rebuilt and checked `sonic-data-lineage-runtime`; status `ok`, `7224` objects, `52437` files, failures `[]`. The package no longer contains a `dbSonicDW` database; remaining literal matches are limited to legitimate object names or raw evidence labels.
- Confluence evidence pass 96: reran `npm run confluence:export`, `npm run confluence:check`, and live `npm run confluence:replace`; export/check passed with warnings `[]` and failures `[]`. The live replacement listed `[AUTO] Database - Sonic_DW` and no `[AUTO] Database - dbSonicDW`; stale generated pages were deleted/replaced under the generated Confluence hierarchy.
