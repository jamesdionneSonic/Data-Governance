...
function tableHasAllowlistKeyword(record, allowlist) {
  const baseText = normalize(
    [record.objectName, record.objectType, record.description, record.rawSnippet].join(' ')
  );
...
export function classifySsisRecords(records, rules = null) {
  ...
  const anomalies = records.filter((record) => {
    ...
    if (
      ...
      hasKeyword(
        normalize(
          [record.objectName, record.displayName, record.description, record.tags].join(' ')
        ),
        allowlist
      )
    ) {
  ...
  return {
    baseline,
    anomaly: anomalies.sort((a, b) => b.edgeCount - a.edgeCount)[0] || records[0],
    anomalies,
  };
}
...
export function classifyTableRecords(records, rules = null) {
  ...
  const baseline =
    records.find((record) => normalize(record.objectName).includes('bt_checklistrecord')) ||
    records[0];
  const anomalies = records.filter((record) => {
    if (record.edgeCount === 0) return true;
    if (record.edgeCount <= review) return false;
