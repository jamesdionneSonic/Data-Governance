...
export const OWNERSHIP_ROLES = Object.freeze({
  owner: {
    ...
    responsibility:
      'Owns business purpose, criticality, certification decisions, and funding accountability.',
    escalationRank: 2,
  },
  steward: {
    label: 'Technical Steward',
    responsibility:
      'Maintains definitions, metadata completeness, lineage evidence, quality triage, and day-to-day governance tasks.',
    escalationRank: 1,
  },
  domain_manager: {
    label: 'Domain Manager',
    responsibility:
      'Resolves escalations across a domain and approves ownership gaps or cross-team conflicts.',
    escalationRank: 3,
  },
  custodian: {
    label: 'Data Custodian',
    responsibility:
      'Owns platform operations, access controls, storage posture, and source-system coordination.',
    escalationRank: 0,
  },
});
...
function resolveOwnership(asset = {}, objects = new Map(), depth = 0, visited = new Set()) {
  ...
  if (Object.keys(roles).length === Object.keys(OWNERSHIP_ROLES).length || depth >= 5) {
    return {
      roles,
      inherited,
      complete: Object.keys(roles).length === Object.keys(OWNERSHIP_ROLES).length,
    };
  }
  if (assetId) visited.add(String(assetId));
  const parent = resolveParentAsset(asset, objects);
  if (parent && !visited.has(String(parent.id))) {
    const parentOwnership = resolveOwnership(
      { ...parent.asset, id: parent.id },
      objects,
      depth + 1,
      visited
    );
...
function replaceState(state = {}) {
  ...
  for (const [key, value] of entriesToMap(state.glossaryReviews))
    glossaryReviewStore.set(key, value);
  for (const [key, value] of entriesToMap(state.trustActions)) trustActionStore.set(key, value);
  for (const [key, value] of entriesToMap(state.publicationChecks))
    publicationChecks.set(key, value);
...
export function createGovernanceTask(payload = {}, user = {}) {
  ...
  const dueDays = Number(
    payload.dueDays || (payload.priority === 'critical' ? 2 : payload.priority === 'high' ? 5 : 14)
  );
...
export function generateStewardshipTasks(objects = new Map(), user = {}, options = {}) {
  ...
  for (const [id, asset] of objects.entries()) {
    ...
    const needsSensitivity = ['table', 'view', 'column', 'dataset', 'report', 'dashboard'].includes(
      assetType
    );
    if (needsSensitivity && !hasValue(asset.sensitivity) && !hasValue(asset.classification))
      gaps.push('sensitivity');
      ...
      createGovernanceTask(
        {
          ...
          owner,
          assigneeRole: ownership.roles.steward
            ? 'steward'
            : ownership.roles.owner
              ? 'owner'
              : null,
  ...
  for (const [id, asset] of objects.entries()) {
    ...
    if (missing.length)
      gaps.push({
        ...
        missing,
        suggestedOwner: assignmentOwnerForAsset({ ...asset, id }, objects),
      });
    assignments.push({
      assetId: id,
      name: assetDisplay(asset),
...
export function buildStewardPortfolio(
  objects = new Map(),
  ...
  for (const [id, asset] of objects.entries()) {
    ...
    const openTasks = listGovernanceTasks({ assetId: id }).filter(
    ...
    if (!hasValue(asset.sensitivity) && !hasValue(asset.classification))
    ...
    assets.push({
      ...
      qualityStatus:
        trust.score >= 75 ? 'healthy' : trust.score >= 50 ? 'watch' : 'needs_attention',
    });
  }
  ...
  return {
    ...
    assetsAtRisk: assets.filter(
      (asset) => asset.qualityStatus !== 'healthy' || asset.metadataGaps.length
    ).length,
    ...
    alerts: assets
      ...
      .map((asset) => ({
        ...
        severity: asset.overdueTaskCount
          ? 'high'
          : asset.metadataGaps.includes('owner') || asset.metadataGaps.includes('steward')
            ? 'high'
            : 'medium',
...
export function planBulkOwnershipAssignment(payload = {}, objects = new Map(), user = {}) {
  ...
  const matchedIds = assetIds.length
    ...
    : [...objects.entries()]
        ...
        .filter(
          ([, asset]) =>
            !filters.type || asset.type === filters.type || asset.object_type === filters.type
        )
        .map(([id]) => id);
  const assignments = {
    ...
    .map((id) => {
      ...
      return {
        ...
        before: Object.fromEntries(
          Object.keys(OWNERSHIP_ROLES).map((role) => [role, roleValue(asset, role)])
        ),
        after: Object.fromEntries(setRoles.map(([role, value]) => [role, value])),
      };
    });
  const task =
    payload.createTask === false || changes.length === 0
      ? null
      : createGovernanceTask(
          {
...
export function addAssetComment(assetId, payload = {}, user = {}) {
  ...
  const comment = {
    ...
    mentions: [
      ...new Set((payload.body || '').match(/@[A-Za-z0-9_-]+(?:\.[A-Za-z0-9_-]+)*/g) || []),
    ],
    tags: asArray(payload.tags),
    status: 'active',
    createdBy: actor,
...
export function listAssetComments(assetId, options = {}) {
  ...
  return options.includeHidden
    ? comments
    : comments.filter((comment) => comment.status === 'active');
}
...
export function recordDecision(assetId, payload = {}, user = {}) {
  ...
  recordOpsEvent(
    ...
    { assetId, decisionId: decision.decisionId },
    decision.createdBy
  );
  persistGovernanceOpsState();
  return decision;
  ...
  return [...objects.entries()]
    .map(([id, asset]) => {
      ...
      const score = Math.min(
        100,
        usageCount * 5 + counts.downstream * 3 + Math.round(trust.score / 4)
      );
      return {
        assetId: id,
        name: assetDisplay(asset),
...
export function recommendRetirementCandidates(objects = new Map(), lineageGraph = new Map()) {
  ...
  return [...objects.entries()]
    .map(([id, asset]) => {
      ...
      if (asset.status === 'deprecated' || asset.lifecycle === 'deprecated')
...
export function detectSchemaChange(payload = {}, user = {}) {
  const before = new Map(
    asArray(payload.beforeColumns).map((col) => [col.name || col.column, col])
  );
...
export function createIncident(payload = {}, user = {}) {
  ...
  recordOpsEvent(
    ...
    { incidentId: incident.incidentId, assetId: incident.assetId },
    actor
  );
  persistGovernanceOpsState();
  return incident;
...
export function listIncidents(filters = {}) {
  ...
  if (filters.assetId)
    incidents = incidents.filter((incident) => incident.assetId === filters.assetId);
...
export function createGlossaryReview(payload = {}, user = {}) {
  ...
  recordOpsEvent(
    'governance.glossary.review_requested',
    { reviewId: review.reviewId, termId: review.termId },
    review.requestedBy
  );
  persistGovernanceOpsState();
  return review;
...
export function decideGlossaryReview(reviewId, payload = {}, user = {}) {
  ...
  recordOpsEvent(
    'governance.glossary.review_decided',
...
export function buildGlossaryHealth(terms = []) {
  ...
  const pendingReviews = [...glossaryReviewStore.values()].filter(
...
export function certifyAsset(assetId, payload = {}, user = {}) {
  ...
  recordOpsEvent(
    'governance.trust.certified',
    { assetId, actionId: action.actionId },
    action.actor
  );
  persistGovernanceOpsState();
  return action;
...
export function buildKpis(objects = new Map(), lineageGraph = new Map()) {
  ...
  const withLineage = [...objects.keys()].filter(
    (id) => lineageCounts(id, lineageGraph).upstream > 0
  ).length;
...
export function buildGovernanceOpsOverview(
  objects = new Map(),
  lineageGraph = new Map(),
  terms = []
) {
  ...
  return {
    ...
    stewardPortfolio: buildStewardPortfolio(objects, lineageGraph, 'all', {
      assetLimit: 10,
      taskLimit: 10,
    }),
