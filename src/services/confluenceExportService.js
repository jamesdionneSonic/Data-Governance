...
const AI_RETRIEVAL_PARENT_TITLE = 'AI Retrieval Artifacts';
const GOVERNED_ASSETS_PARENT_TITLE = 'Governed Assets';
...
function renderReadmePage({ generatedAt, rootPageUrl, summary }) {
  return [
    ...
    '',
    '> This page set is maintained by the Data Governance Platform. Edit source markdown or extractor logic, not the published Confluence pages.',
    '',
    '## Purpose',
    '',
...
function pageRecord(title, fileInfo, extra = {}) {
  return {
    title,
    content_type: 'page',
    labels: ['data-lineage', 'lineage-catalog', ...(extra.labels || [])],
...
export async function buildConfluenceExport(options = {}) {
  ...
  const pageDefinitions = [
    {
      title: AI_RETRIEVAL_PARENT_TITLE,
      relativePath: path.join(artifactRoot, 'pages', 'ai-retrieval-artifacts.md'),
      content: () =>
        [
          '# AI Retrieval Artifacts',
          '',
          'Technical lineage lookup pages for Codex, Rovo, validation, and troubleshooting.',
          '',
          'Use these pages after the human-facing catalog identifies the object, database, or subject area to investigate.',
          '',
          '## Contents',
          '',
          '- Object locator pages',
          '- Lineage quick context pages',
          '- Catalog shard pages',
          '- Catalog manifest and object index',
          '- Database browse pages',
          '- Governed asset technical pages',
          '',
        ].join('\n'),
      labels: ['ai-retrieval-artifacts'],
    },
    {
      title: GOVERNED_ASSETS_PARENT_TITLE,
      relativePath: path.join(artifactRoot, 'pages', 'governed-assets.md'),
      content: () =>
        [
          '# Governed Assets',
          '',
          'Technical detail pages for selected high-value lineage assets.',
          '',
          'These pages are generated for support and validation. Human-facing high-value summaries belong under the human catalog.',
          '',
        ].join('\n'),
      labels: ['governed-assets'],
      parentTitle: AI_RETRIEVAL_PARENT_TITLE,
    },
    {
  ...
  const governedAssetRecords = await mapWithConcurrency(
    ...
    async (definition) => {
      ...
      return pageRecord(definition.title, fileInfo, {
        ...
        parentTitle: GOVERNED_ASSETS_PARENT_TITLE,
      });
    }
  );
