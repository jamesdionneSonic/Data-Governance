...
function getRedirectUri(req) {
  const explicit = process.env.ENTRA_REDIRECT_URI;
  if (explicit) {
    return explicit;
  }
...
router.post('/login', (req, res) => {
  ...
  if (!email) {
    ...
    return res.json({
      ...
      redirectUrl: `https://login.microsoftonline.com/${process.env.ENTRA_TENANT_ID || 'common'}/oauth2/v2.0/authorize?client_id=${process.env.ENTRA_CLIENT_ID}&redirect_uri=${encodeURIComponent(getRedirectUri(req))}&response_type=code&scope=openid%20email%20profile`,
    });
  }
