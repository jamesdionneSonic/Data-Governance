...
| `@SourceSystemName` | varchar | No     | No      |
...
---- 08/13/2020 - jo - added uptypekey and leadsourcekey
---- 11/12/2020 - jo - added leadsubsourcekey
---- 04/22/2021 - DMD - added UPDATE for Meta_RowIsCurrent
---- 06/10/2021 - DMD - added ISNULL for LeadSourceKey and LeadSubSourceKey
*************************************************************/
...
-----------------------------update 01/31/2018 RAJ/LANA----------------------
	...
	MERGE dbo.FactOpportunity AS fac
		...
		(	SELECT 	FactOpportunityKey
					...
					,DMSLegacyDealID
					,UpTypeKey
					,ISNULL(LeadSourceKey,-1) AS LeadSourceKey
					,ISNULL(LeadSubSourceKey,-1) AS LeadSubSourceKey
			FROM ETL_Staging.clean.FactOpportunity
		) AS opp
		...
		WHEN MATCHED
			THEN UPDATE
				SET  fac.EntityKey = opp.EntityKey
				    ,fac.FocusCustomerKey = opp.DimCustomerID --- added 01/24/2019
				    ,fac.SourceKey = opp.SourceKey --  added 12/14/2018
					,fac.PrimarySalesAssociateKey   = opp.PrimarySalesAssociate  -- added 03/21/2018
					,fac.DateProspectInKey  = opp.DateProspectInKey -- added 03/21/2018
					,fac.LastActivityDateKey  = opp.LastActivityDateKey
					,fac.LastActivityTimeKey = opp.LastActivityTimeKey-- added 03/21/2018
					,fac.DateSoldKey = opp.DateSoldKey-- added 03/21/2018
					,fac.DMSDateSoldKey = opp.DMSDateSoldKey-- added 03/21/2018
					,fac.InactiveDateKey = opp.InactiveDateKey-- added 03/21/2018
					,fac.VehicleKey = opp.VehicleKey-- added 03/21/2018
					,fac.TradeInVehicleKey = opp.TradeInVehicleKey-- added 03/21/2018
					,fac.LeadStatusKey = opp.LeadStatusKey-- added 03/21/2018
					,fac.DealTypeKey = opp.DealTypeKey-- added 03/21/2018
					,fac.PurchaseType = opp.PurchaseType-- added 05/16/2019
					,fac.DMSLegacyDealID = opp.DMSLegacyDealID	--added 06/20/2019
					,fac.NewVehicleFlag = opp.NewVehicleFlag-- added 03/21/2018
					,fac.LeadCount = opp.LeadCount-- added 03/21/2018
					,fac.DuplicateLeadCount = opp.DuplicateLeadCount-- added 03/21/2018
					,fac.LeadCost = opp.LeadCost-- added 03/21/2018
					,fac.DaysActive = opp.DaysActive-- added 03/21/2018
					,fac.SoldCount = opp.SoldCount-- added 03/21/2018
					,fac.DMSSoldCount = opp.DMSSoldCount-- added 03/21/2018
					...
					,fac.TotalSurveySoldCount =  opp.TotalSurveySoldCount -- added 03/21/2018
					,fac.Meta_RowLastChangedDate = opp.MetaRowLastChangeDate
					,fac.ETLExecution_ID = opp.ETLExecutionID
					, fac.UpTypeKey = opp.UptypeKey
					, fac.LeadSourceKey = opp.LeadSourceKey
					, fac.LeadSubSourceKey = opp.LeadSubSourceKey
					,fac.Meta_RowIsCurrent = opp.MetaRowIsCurrent		-- added 04/22/2021
		WHEN NOT MATCHED
			THEN INSERT(EntityKey,
					...
					VALUES(opp.EntityKey,
							...
							, opp.UptypeKey
							, opp.LeadSourceKey
							, opp.LeadSubSourceKey
								----left (opp.MetaNaturalKey, charindex('-', opp.MetaNaturalKey) - 1) --- updated raj 04-19-2018
							  );
END
	  ...
	  -- where factOpportunityKey is null
	   ...
	   --******************* Update FactOpportunityKey values in FactActivity records *************************
		...
		on opp.Meta_NaturalKey = (cast(cl.lDealID as varchar)) --- changed 04/19/2018 - raj (cast(opp.lDealID as varchar))
