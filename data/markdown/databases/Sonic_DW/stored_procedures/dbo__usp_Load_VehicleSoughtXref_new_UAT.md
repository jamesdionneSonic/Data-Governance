---
name: usp_Load_VehicleSoughtXref_new_UAT
database: Sonic_DW
type: procedure
schema: dbo
owner: Data Team
...
- **dbo.DimVehicleSoughtXref_UAT** (U )
...
-- =================================================
CREATE PROCEDURE [dbo].[usp_Load_VehicleSoughtXref_new_UAT] (
...
BEGIN
	...
	MERGE dbo.DimVehicleSoughtXref_UAT AS [tgt]
	USING (
			...
			FROM etl_staging.[wrk].[Dim_VehicleSoughtXref_Staging] opp INNER JOIN dbo.FactOpportunity_UAT fac
					---ON CAST(opp.lOppID AS VARCHAR) = LEFT(fac.meta_naturalKey, CHARINDEX('-', fac.meta_naturalKey) - 1)
					ON CAST(opp.lOppID AS VARCHAR) = fac.meta_naturalKey
