...
-- 04/28/2025:  DMD - Added start and end date parameters
-- ====================================================================================================================================================================================================
CREATE PROC [dbo].[usp_LoadFactTrafficSummarySubSource_HIST]
--(
--	@StartDate DATE
--	,@EndDate DATE
...
SELECT DISTINCT
  ...
  on dd.fulldate = CASE WHEN convert(date,agg.eventdate) = CONVERT(DATE,(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-0, 0))) THEN CONVERT(DATE,(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-0, -1)))
					WHEN convert(date,agg.eventdate) = CONVERT(DATE,(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-1, -1))) THEN CONVERT(DATE,(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-1, 0)))
					ELSE convert(date,agg.eventdate) END
  --where agg.lchildcompanyid = 634
  --WHERE CONVERT(DATE,agg.EventDate) BETWEEN @StartDate AND @EndDate
  group by
  	EntityKey
	  ,isnull(UpTypeKey,-1)
      ,[szNewUsed]
