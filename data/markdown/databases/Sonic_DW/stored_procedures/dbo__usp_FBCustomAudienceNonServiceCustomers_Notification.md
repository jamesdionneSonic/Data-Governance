---
name: usp_FBCustomAudienceNonServiceCustomers_Notification
database: Sonic_DW
type: procedure
schema: dbo
owner: Data Team
...
CREATE PROCEDURE [dbo].[usp_FBCustomAudienceNonServiceCustomers_Notification]
...
AS
     BEGIN
         ...
         , @ToLine NVARCHAR(MAX)= @ToLine1, @Body_Line1 NVARCHAR(MAX)= 'Facebook Custom Audience Non Service Customers Upload Summary',
     ...
     ' End Time :'+CONVERT(VARCHAR(20), GETDATE(), 100), @SourceCount INT, @FailedCount INT, @SuccessCount INT, @Metafiledate DATETIME, @NewCount INT;
         ...
         (
             ...
             FROM ETL_Staging.dbo.StgFBAudienceNonServiceCustomers
         );
         SET @SourceCount =
         (
             SELECT COUNT(*)
         ...
         (
             SELECT COUNT(*)
             FROM ETL_Staging.dbo.FailedAudienceIDNonServiceCustomer
         );
     SET @NewCount =
     (
     Select COUNT(*)
     FROM FBCustomAudience WHERE AudienceType = 'NonServiceCustomers'
