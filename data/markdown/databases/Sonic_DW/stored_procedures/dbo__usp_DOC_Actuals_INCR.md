...
-- =============================================
CREATE PROCEDURE [dbo].[usp_DOC_Actuals_INCR]
...
FROM
 ...
 SELECT 	a11.[EntityKey]  EntityKey,
			...
			a13.[GroupElementSort]  GroupElementSort,
	...
	WHERE	a11.[AccountingDateKey] >= @closedatekey

	GROUP BY	a11.[EntityKey],
		a11.[AccountingDateKey],
		a13.[GroupElementSort],
		a14.[GroupElement],
		a14.[GroupSubElement]
) A1
...
--Doc_Actual_Summary metric table--

DELETE FROM [dbo].[Doc_Actual_Summary] WHERE DateKey >= @closedatekey
...
SELECT EntityKey, DateKey, 451, (Expenses + Other) FROM CalcTableS
	UNION ALL
--SELECT EntityKey, DateKey, 460, (Other) FROM CalcTableS
--	UNION ALL
SELECT EntityKey, DateKey, 500, ((isnull(DealershipGross,0) - isnull(Expenses,0)) - isnull(Other,0)) FROM CalcTableS
	UNION ALL
