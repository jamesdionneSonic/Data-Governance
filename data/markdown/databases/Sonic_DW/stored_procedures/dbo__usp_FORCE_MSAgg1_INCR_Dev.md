---
name: usp_FORCE_MSAgg1_INCR_Dev
database: Sonic_DW
type: procedure
schema: dbo
owner: Data Team
...
- **dbo.DM_FORCE_SUMMARY_Dev** (U )
- **dbo.DM_FORCE_Summary_short_dev** (U )
...
--    exec [dbo].[usp_FORCE_MSAgg1_INCR_Dev] '20140308'
...
-- =============================================
CREATE PROCEDURE [dbo].[usp_FORCE_MSAgg1_INCR_Dev] @closedatekey int
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DM_FORCE_Summary_short_dev]') AND type in (N'U'))
	drop table DM_FORCE_Summary_short_dev
--FORCETriggerInc.scp

delete from DM_FORCE_Summary_Dev where DateKey >= @closedatekey
...
--delete from DM_FORCE_Summary_Dev where EntDealerLvl1 = 'Global BMW' and DateKey >=20130308
insert into DM_FORCE_Summary_Dev
select * from DM_FORCE_Summary_short_dev
*/
...
;with gopa1 as
 (select	a11.[ServiceAdvisorKey]  ServiceAdvisorKey,
	...
	where	(a11.[ROStatus] = 1 and CloseDateKey >= @closedatekey)
 ...
 (select	a12.[ServiceAdvisorKey]  ServiceAdvisorKey,
		...
		sum((Case when a11.[LineItemType] in ('WTY') then a11.[LineItemCount] else NULL end))  WTYLICount,
		sum((Case when a11.[LineItemType] in ('CP', 'OTH') then a11.[LaborSale] else NULL end))  CPLaborSales,
		sum((Case when a11.[LineItemType] in ('CP', 'OTH') then a11.[PartsSale] else NULL end))  CPPartsSales,
		sum((Case when a11.[LineItemType] in ('CP', 'OTH') then a11.[PartsCost] else NULL end))  CPPartsCost,
		sum((Case when a11.[LineItemType] in ('CP', 'OTH') then a11.[LaborCost] else NULL end))  CPLaborCost,
		sum((Case when a11.[LineItemType] in ('CP', 'OTH') then a11.[MiscCost] else NULL end))  CPMiscCost,
		sum((Case when a11.[LineItemType] in ('CP', 'OTH') then a11.[MiscSale] else NULL end))  CPMiscSales,
		...
		sum((Case when a11.[LineItemType] in ('OTH') then a11.[LineItemCount] else NULL end))  OTHLICount,
		...
		sum((Case when a11.[LineItemType] in ('INT') then a11.[LineItemCount] else NULL end))  INTLICount,
		sum((Case when coalesce(a11.[OpcMenu], 'UKN') in ('Menu', 'Menu Basic', 'Menu Rec', 'Menu Value') then a11.[SoldHours] else NULL end))  MenuHours,
		...
		sum((Case when a11.[originappl] in ('3PA_DEALERLOGIX', 'DealerLOGIX') then a11.[ItemCount] else NULL end))  CountofDLX
	from	dbo.vw_Fact_ServiceDetail	a11
		join	dbo.vw_Fact_Service	a12
		  on 	(a11.[ServiceKey] = a12.[ServiceKey])
	where	(a12.[ROStatus] = 1 and CloseDateKey >= @closedatekey)
