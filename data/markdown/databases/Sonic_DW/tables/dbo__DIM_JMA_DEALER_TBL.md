...
| ------------------------- | --------- | -------- | -------- | --------------- | ----------- |
| `JMA_DEALER_ID`           | bigint    |          | ✓        |                 |             |
| `JMA_FWS_NUMBER`          | int       | ✓        |          |                 |             |
| `JMA_FWS_DESC`            | varchar   | ✓        |          |                 |             |
| `ACTIVE_IND`              | int       |          |          | ((1))           |             |
...
- **Primary Key**: PK_DIM_JMA_DEALER_TBL
  - Columns: JMA_DEALER_ID

## Indexes

- **PK_DIM_JMA_DEALER_TBL** (NONCLUSTERED)
  - Unique: Yes
  - Primary Key: Yes
  - Key Columns: JMA_DEALER_ID ASC
...
- **column_match**: dbo.DIM_JMA_CONDITION_TBL → dbo.DIM_JMA_DEALER_TBL
  ...
  - Evidence: Exact column name match: "etlexecution_id" in both tables
  - Column: `ETLEXECUTION_ID` → `ETLEXECUTION_ID`
- **column_match**: dbo.DIM_JMA_COVERAGE_LENGTH_TBL → dbo.DIM_JMA_DEALER_TBL
  - Confidence: 80%
  - Evidence: Exact column name match: "etlexecution_id" in both tables
  - Column: `ETLEXECUTION_ID` → `ETLEXECUTION_ID`
