...
| ------------------------- | --------- | -------- | -------- | --------------- | ----------- |
| `JMA_CONDITION_ID`        | bigint    |          | ✓        |                 |             |
| `JMA_CONDITION_CODE`      | varchar   | ✓        |          |                 |             |
| `JMA_CONDITION_DESC`      | varchar   | ✓        |          |                 |             |
| `ACTIVE_IND`              | int       |          |          | ((1))           |             |
...
- **Primary Key**: PK_DIM_JMA_CONDITION_TBL
  - Columns: JMA_CONDITION_ID

## Indexes

- **PK_DIM_JMA_CONDITION_TBL** (NONCLUSTERED)
  - Unique: Yes
  - Primary Key: Yes
  - Key Columns: JMA_CONDITION_ID ASC
...
- **column_match**: dbo.DIM_JMA_CONDITION_TBL → dbo.DIM_JMA_COVERAGE_LENGTH_TBL
  - Confidence: 80%
  - Evidence: Exact column name match: "etlexecution_id" in both tables
  - Column: `ETLEXECUTION_ID` → `ETLEXECUTION_ID`
- **column_match**: dbo.DIM_JMA_CONDITION_TBL → dbo.DIM_JMA_COVERAGE_PLAN_TYPE_TBL
  ...
  - Evidence: Exact column name match: "etlexecution_id" in both tables
  - Column: `ETLEXECUTION_ID` → `ETLEXECUTION_ID`
- **column_match**: dbo.DIM_JMA_CONDITION_TBL → dbo.DIM_JMA_DEALER_TBL
  - Confidence: 80%
  - Evidence: Exact column name match: "etlexecution_id" in both tables
  - Column: `ETLEXECUTION_ID` → `ETLEXECUTION_ID`
