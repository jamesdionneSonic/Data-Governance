...
| `JMA_FINANCE_TYPE_CODE`   | varchar   | ✓        |          |                 |             |
| `JMA_FINANCE_TYPE_DESC`   | varchar   | ✓        |          |                 |             |
| `ACTIVE_IND`              | int       |          |          | ((1))           |             |
...
- **Primary Key**: PK_DIM_JMA_FINANCE_TYPE_TBL
  - Columns: JMA_FINANCE_TYPE_ID

## Indexes

- **PK_DIM_JMA_FINANCE_TYPE_TBL** (NONCLUSTERED)
  - Unique: Yes
  - Primary Key: Yes
  - Key Columns: JMA_FINANCE_TYPE_ID ASC
...
- **column_match**: dbo.DIM_JMA_CONDITION_TBL → dbo.DIM_JMA_FINANCE_TYPE_TBL
  - Confidence: 80%
  - Evidence: Exact column name match: "etlexecution_id" in both tables
  - Column: `ETLEXECUTION_ID` → `ETLEXECUTION_ID`
- **column_match**: dbo.DIM_JMA_COVERAGE_LENGTH_TBL → dbo.DIM_JMA_FINANCE_TYPE_TBL
  - Confidence: 80%
  - Evidence: Exact column name match: "etlexecution_id" in both tables
  - Column: `ETLEXECUTION_ID` → `ETLEXECUTION_ID`
- **column_match**: dbo.DIM_JMA_COVERAGE_PLAN_TYPE_TBL → dbo.DIM_JMA_FINANCE_TYPE_TBL
  ...
  - Evidence: Exact column name match: "etlexecution_id" in both tables
  - Column: `ETLEXECUTION_ID` → `ETLEXECUTION_ID`
- **column_match**: dbo.DIM_JMA_DEALER_TBL → dbo.DIM_JMA_FINANCE_TYPE_TBL
  - Confidence: 80%
  - Evidence: Exact column name match: "etlexecution_id" in both tables
  - Column: `ETLEXECUTION_ID` → `ETLEXECUTION_ID`
