...
e.EntADPCompanyID, Isnull(e.EntCora_Account_ID, - 1) AS EntCoraAccountID, f.EntityKey, e.EntLineOfBusiness, MAX(d .DealTypeCode) AS DealTypeFI/*NEW COLUMN*/ , CASE WHEN a.FIAccount IN (9088)
THEN 'Used' ELSE a.FIAccountClassification END AS FIAccountClassification/*only works for EP, 9088 excluded from Sonic			    */ , f.StockNo AS StockNumber, SUM(CASE WHEN a.FIAccountType = 'C' AND
...
WHERE        /*-ad.FullDate between DateAdd(m, -1, DATEADD(m, DATEDIFF(m, 0, GETDATE()), 0)) and eomonth(getdate())*/ ad.FullDate >= '2021-01-01' AND FIAccount NOT IN (9030, 9032, 9088)
