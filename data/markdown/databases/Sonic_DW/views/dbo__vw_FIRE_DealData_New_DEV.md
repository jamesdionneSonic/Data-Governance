...
CREATE VIEW [dbo].[vw_FIRE_DealData_New_DEV]
AS
...
WITH    FFGU
          AS ( SELECT   f.StockNo
               ...
               WHERE   AccountingDateKey >= CAST(REPLACE(convert(varchar(10),dateadd(mm,-18,dateadd(dd,(DATEPART(dd,getdate())-1) * -1,getdate())),120),'-','') AS INT)
...
---------additional column added - Raj 06/30/2017--------------
           ,SUM(CASE WHEN FIAccountType = 'S'
               ...
               WHERE    f.AccountingDateKey >= CAST(REPLACE(convert(varchar(10),dateadd(mm,-18,dateadd(dd,(DATEPART(dd,getdate())-1) * -1,getdate())),120),'-','') AS INT)
