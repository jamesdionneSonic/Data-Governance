...
Generated: 2026-06-19T08:45:26.966Z
...
| Platform              | ADF                                                                                                                                                                                                 |
| Asset type            | Pipeline                                                                                                                                                                                            |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-dw-marketing-prod/pipelines/pl_Marketing_AWS_Export`       |
| Support role          | root orchestrator                                                                                                                                                                                   |
...
| Primary target/output | pl_eLead_Org_Mappings, pl_DMS_Org_Mappings, pl_mdp_dms_individual, pl_mdp_dms_purchaseinfo, pl_mdp_eLead_activity                                                                                   |
...
1. Check the latest pl_Marketing_AWS_Export parent run, then identify any failed child pipeline before rerunning individual child steps.
...
4. Confirm target datasets or child pipelines completed: `pl_eLead_Org_Mappings`, `pl_DMS_Org_Mappings`, `pl_mdp_dms_individual`, `pl_mdp_dms_purchaseinfo`, `pl_mdp_eLead_activity`, `pl_mdp_elead_individual`, `pl_mdp_elead_vehicleinterest`.
...
| Child pipelines   | pl_eLead_Org_Mappings, pl_DMS_Org_Mappings, pl_mdp_dms_individual, pl_mdp_dms_purchaseinfo, pl_mdp_eLead_activity, pl_mdp_elead_individual, pl_mdp_elead_vehicleinterest          |
...
| ---------------------------------- | ------------------------ | ------------------------ | ------------------------ |
| Execute ELead Org Mappings         | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
| Execute DMS Org Mappings           | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
...
| Execute DMS Individual             | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
| Set DMS Individual Failure         | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
| Execute DMS purchase info          | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
| Execute eLead activity             | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
| Execute elead individual           | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
| Execute elead vehicle interest     | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
