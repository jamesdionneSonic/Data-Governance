...
1. Dataset `DataSet1` (Text): SELECT DISTINCT o.Name 'Dealership', COUNT(CASE WHEN vi.curr_status_id IN (201,202,203,205) --PA,IGT,IS,PW THEN vi.Vehicle_ID END) 'Level' FROM [SIMS6200].[dbo].[Vehicle_Inventory] vi --JOIN FOR DEALERSHIPS inner join SIMS6200.dbo.Organization o --UPDATED 8.12.13 ON o.Org_ID = vi.Store_ID WHERE o.Is_Act = 1 --ACTIVE ST...
