...
| Platform              | SSRS                                                                                                                                                                              |
| Asset type            | Report                                                                                                                                                                            |
| Native path           | `/RTC/R3 - Corporate Sold Own Tracker`                                                                                                                                            |
| Support role          | Review candidate report                                                                                                                                                           |
| Business process      | Use this when the business needs a vehicle inventory or pricing view for operational follow-up, exception review, or availability monitoring. The report is filtered by From, To. |
...
Use this when the business needs a vehicle inventory or pricing view for operational follow-up, exception review, or availability monitoring. The report is filtered by From, To.
...
1. Dataset `DataSet1` (Text): SELECT COUNT(CASE WHEN vi.curr_status_id IN (301,302,304) THEN vi.Curr_Status_ID END) 'Total Sold' INTO #sold FROM [SIMS6200].[dbo].[Vehicle_Inventory] vi --JOIN FOR DEALERSHIPS join SIMS6200.dbo.Organization o --UPDATED 8.12.13 ON o.Org_ID
...
```sql
SELECT     COUNT(CASE   WHEN vi.curr_status_id IN (301,302,304)     THEN vi.Curr_Status_ID END) 'Total Sold'  INTO #sold    FROM [SIMS6200].[dbo].[Vehicle_Inventory] vi  --JOIN FOR DEALERSHIPS join SIMS6200.dbo.Organization o --UPDATED 8.12.13 ON o.Org_ID
```
