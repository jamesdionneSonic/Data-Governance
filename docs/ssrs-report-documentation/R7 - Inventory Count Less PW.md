...
| Platform              | SSRS                                                                                                                                          |
| Asset type            | Report                                                                                                                                        |
| Native path           | `/RTC/R7 - Inventory Count Less PW`                                                                                                           |
...
1. Dataset `DataSet1` (Text): SELECT DISTINCT o.Name 'Dealership', COUNT(CASE WHEN vi.curr_status_id IN (201,202,203) THEN vi.Vehicle_ID END) 'Total Inventory' FROM [SIMS6200].[dbo].[Vehicle_Inventory] vi --JOIN FOR DEALERSHIPS inner join SIMS6200.dbo.Organization o --UPDAT
...
```sql
SELECT DISTINCT o.Name 'Dealership',  COUNT(CASE    WHEN vi.curr_status_id IN (201,202,203)    THEN vi.Vehicle_ID END) 'Total Inventory'    FROM  [SIMS6200].[dbo].[Vehicle_Inventory] vi  --JOIN FOR DEALERSHIPS inner join SIMS6200.dbo.Organization o --UPDAT
```
