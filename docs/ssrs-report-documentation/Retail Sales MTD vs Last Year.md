...
| Platform              | SSRS                                                                                                                                                                                        |
| Asset type            | Report                                                                                                                                                                                      |
| Native path           | `/BI - FPnA/Retail Sales MTD vs Last Year`                                                                                                                                                  |
| Support role          | Review candidate report                                                                                                                                                                     |
| Business process      | Use this when the business needs a vehicle inventory or pricing view for operational follow-up, exception review, or availability monitoring. The report is filtered by Beg Date, End Date. |
...
Use this when the business needs a vehicle inventory or pricing view for operational follow-up, exception review, or availability monitoring. The report is filtered by Beg Date, End Date.
...
1. Dataset `Retail_Sales_MTD_lastyear` (Text): WITH VSC AS (SELECT DISTINCT c.related_acctg_cora_acct_id, v.stockno, v.dealno, v.vin AS VIN, v.year AS ModelYear, v.makename AS Make, v.modelname AS Model FROM [COR-SQL-02].DMS.dbo.vehiclesalescurrent AS v INNER J
...
```sql
WITH VSC AS (SELECT DISTINCT c.related_acctg_cora_acct_id, v.stockno, v.dealno, v.vin AS VIN, v.year AS ModelYear, v.makename AS Make, v.modelname AS Model                                FROM            [COR-SQL-02].DMS.dbo.vehiclesalescurrent AS v INNER J
```
