...
- Added the connector metadata profile framework for Salesforce, cloud storage, catalog platforms, pipeline/orchestration tools, dbt, and code repositories. New endpoints `/api/v1/connectors/:id/metadata-profile/plan` and `/api/v1/connectors/:id/metadata-profile/run` profile metadata-only inventory, storage locations, schemas, columns, classifications, glossary terms, pipelines, tasks, jobs, datasets, connections, schedules, code assets, reports, dashboards, lineage, coverage gaps, Confluence summaries, and assistant-ready English.
- Deferred OpenAPI, Kafka, and SAP profile support to the next pass with explicit `CONNECTOR_CONFIG_ERROR` remediation.
