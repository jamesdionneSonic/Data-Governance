...
FROM            (SELECT        a0.EmployeeID, a0.ADName, a0.ADEmail, COALESCE (a3.EntityKey, a1.EntityKey, - 1) AS EntityKey, a1.AsoFirstName, a1.AsoLastName, a1.AsoecJobtitle, a1.AsoJobCode, a2_1.MSTRMetadataUserID,
                                                    a1.AssociateKey
                          FROM            (SELECT        a1_1.AsoEmployeeNumber, (CASE WHEN a1_3.entlineofbusiness = 'SONIC' THEN a1_2.EntityKey ELSE - 1 END) AS EntityKey, a1_2.AsoFirstName, a1_2.AsoLastName, a1_2.AsoecJobtitle,
                                                    ...
                                                    dbo.vw_Dim_ADUsers AS a0 LEFT OUTER JOIN
                                                        (SELECT        EmployeeID, EntityKey
                                                          ...
                                                          WHERE        (UE_isDefault = 1) AND (EntLineOfBusiness = 'Sonic')) AS a3 ON TRY_CAST(CASE WHEN IsNumeric(a0.EmployeeID)  = 1 THEN a0.EmployeeID ELSE NULL END AS bigint) = a3.EmployeeID LEFT OUTER JOIN
