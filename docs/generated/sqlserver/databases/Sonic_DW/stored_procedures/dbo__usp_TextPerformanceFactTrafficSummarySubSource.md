---
name: usp_TextPerformanceFactTrafficSummarySubSource
database: Sonic_DW
type: procedure
schema: dbo
owner: Data Team
...
-- =================================================
CREATE PROC [dbo].[usp_TextPerformanceFactTrafficSummarySubSource]
--(
...
-------------Begin MERGE-----------------------
    ...
    SELECT tp.EntityKey
      ...
      ,dls.LeadSourceKey
      ,tp.szSource
      ,dlss.LeadSubSourceKey
      ,tp.SzSubSource
      ,tp.FiscalMonthDateKey AS EventDateKey
    ...
    JOIN (SELECT DISTINCT MAX(LeadSourceKey) LeadSourceKey, LeadSourceName FROM sonic_dw.dbo.DimLeadSource GROUP BY LeadSourceName) dls
     on LTRIM(RTRIM(ISNULL(tp.szSource,'UNKNOWN'))) = LTRIM(RTRIM(dls.LeadSourceName))
    JOIN (SELECT DISTINCT MAX(LeadSubSourceKey) LeadSubSourceKey, LeadSubSourceName, eLeadChildCompanyID FROM sonic_dw.dbo.DimLeadSubSource GROUP BY eLeadChildCompanyID,LeadSubSourceName) dlss
     on LTRIM(RTRIM(isnull(tp.SzSubSource,'UNKNOWN'))) = LTRIM(RTRIM(dlss.LeadSubSourceName))
     and tp.lchildcompanyid = dlss.eleadChildCompanyId
    ...
    GROUP BY tp.EntityKey
      ...
      ,FiscalMonthDateKey
		...
		AND ISNULL(src.LeadSourceKey,-1) = tgt.LeadSourceKey
		AND ISNULL(src.LeadSubSourceKey,-1) = tgt.LeadSubSourceKey
	WHEN MATCHED
					AND (
	...
	WHEN NOT MATCHED
		THEN
			...
			VALUES (
					...
					,src.TextApptNoPhone
					,src.TextSoldNoPhone
					,(SELECT MAX([ETLExecution_ID]) FROM Sonic_DW.dbo.FactTrafficSummarySubsource)	--[ETLExecution_ID]
