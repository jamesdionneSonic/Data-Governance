...
AS
     BEGIN
         ...
         , @ToLine NVARCHAR(MAX)= @ToLine1, @Body_Line1 NVARCHAR(MAX)= 'Facebook Custom Audience CarCash Web Sold Upload Summary',
     ...
     ' End Time :'+CONVERT(VARCHAR(20), GETDATE(), 100), @SourceCount INT, @FailedCount INT, @SuccessCount INT, @Metafiledate DATETIME, @NewCount INT;
         ...
         (
             ...
             FROM ETL_Staging.dbo.[StgFBAudienceCarCashWebSoldAudience]
         );
         SET @SourceCount =
         (
             SELECT COUNT(*)
         ...
         (
             SELECT COUNT(*)
             FROM ETL_Staging.dbo.FailedAudienceIDCarCashWebSold
         );
     SET @NewCount =
     (
     Select COUNT(*)
     FROM FBCustomAudience WHERE AudienceType = 'FBAudienceCCWebSold'
