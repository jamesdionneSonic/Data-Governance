---
name: usp_FIRE_BuildSummaryFact_DEV
database: Sonic_DW
type: procedure
schema: dbo
owner: Data Team
...
-- =====================================================================================
CREATE   PROCEDURE [dbo].[usp_FIRE_BuildSummaryFact_DEV] (@StartDateKey int,@EndDateKey int)
AS
BEGIN
	SET NOCOUNT ON;
		...
		begin
			begin transaction
				delete
					top (10000)
				from
					[V1-SQL-01\INST1].[Sonic_DW].dbo.factFIRE_A
				where
	...
	from [V1-SQL-01\INST1].[Sonic_DW].dbo.factFIRE where dealno <> '' and AccountingDateKey between @StartDateKey and @EndDateKey
	)
	...
	,CTB as
	(
	select distinct entitykey,dealno,stockno,salesperson1key as SalesPersonKey from [V1-SQL-01\INST1].[Sonic_DW].dbo.factfire
	where SalesPerson1Key <> -1 and dealno <> ''  and AccountingDateKey between @StartDateKey and @EndDateKey
	union
	select distinct entitykey,dealno,stockno,salesperson2key as SalesPersonKey from [V1-SQL-01\INST1].[Sonic_DW].dbo.factfire
	where SalesPerson2Key <> -1 and dealno <> '' and AccountingDateKey between @StartDateKey and @EndDateKey
	union
	select distinct entitykey,dealno,stockno,salesperson3key as SalesPersonKey from [V1-SQL-01\INST1].[Sonic_DW].dbo.factfire
	...
	insert into [V1-SQL-01\INST1].[Sonic_DW].dbo.factfire_A
	(
		EntityKey,
		DealNo,
		FIMgrKey,
	...
	SELECT
			...
			VSC_RowLastUpdated
		FROM
		  [V1-SQL-01\INST1].[Sonic_DW].dbo.factFIRE FF
