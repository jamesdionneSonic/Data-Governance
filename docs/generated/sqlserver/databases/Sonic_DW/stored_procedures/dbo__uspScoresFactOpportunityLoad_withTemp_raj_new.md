---
name: uspScoresFactOpportunityLoad_withTemp_raj_new
database: Sonic_DW
type: procedure
schema: dbo
owner: Data Team
...
---- =============================================
CREATE PROCEDURE [dbo].[uspScoresFactOpportunityLoad_withTemp_raj_new] (
...
SET @now = getdate();
	...
	SELECT ISNULL(DE.EntityKey, -1) AS [EntityKey]
			...
			,CASE
				WHEN fop.fiwipstatuscode = 'B' then 100000051
				WHEN fop.fiwipstatuscode = 'F' then 100000052
				ELSE stg.sa_opportunitystate END AS sa_opportunitystate
			,stg.sa_dmsstatus
			,SUM(CASE
					WHEN stg.statuscode in ( 100000024,
										100000025,
										100000027,
										100000043,
										100000044,
			...
			--		END) AS [DMSSoldCount]
			,SUM(CASE
				WHEN fop.fiwipstatuscode in ('F','B')
					THEN 1
				ELSE 0
				END) AS [DMSSoldCount]
			,stg.sa_beback
		...
		FROM  [ETL_Staging].[dbo].[StgFactOppScores] stg	--
				...
				------------------new add on-----------------
				left join [ETL_Staging].[dbo].[Sa_Deal_fire_opp] as fop
				ON fop.sa_opportunityid = stg.opportunityid
				----and fop.AccountingDateKey = convert(varchar(10),stg.modifiedon,112)  --------------- added new 02/14/2018
...
--************************************************************	MIGRATE ELEAD OPPORTUNITIES TO SCORES AS NEEDED ***************************************
	...
	SELECT A.EntityKey
		...
		,ISNULL(CASE
				WHEN A.statuscode in ( 100000024,
										100000025,
										100000027,
										100000043,
										100000044,
		...
		,ISNULL(A.sa_opportunitystate_displayname, 'UNKNOWN') AS sa_opportunitystate_displayname
	...
	FROM #MergeSource AS A
		...
		LEFT JOIN Sonic_DW.dbo.Dim_Date AS DD3 ON cast(A.[sa_closedate] as date) = DD3.FullDate
			------------------new add on-----------------
	    left join [ETL_Staging].[dbo].[Sa_Deal_fire_opp] as fop  ON fop.sa_opportunityid = A.opportunityid
															    ----and fop.AccountingDateKey = convert(varchar(10),A.modifiedon,112)  --------------- added new 02/14/2018
		----------LEFT JOIN Sonic_DW.dbo.Dim_LeadStatus AS DLS ON A.sa_opportunitystate = DLS.LeadStatusCode
		LEFT JOIN Sonic_DW.dbo.Dim_LeadStatus AS DLS ON A.StatusCode = DLS.LeadStatusCode ---------new update
