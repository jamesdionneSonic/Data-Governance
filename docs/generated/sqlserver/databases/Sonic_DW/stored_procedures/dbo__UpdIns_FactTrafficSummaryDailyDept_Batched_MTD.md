...
------------------------------------------------------------------
IF OBJECT_ID('tempdb..#StageTSDDept_MTD_SourceData') IS NOT NULL DROP TABLE #StageTSDDept_MTD_SourceData;
...
SELECT DISTINCT
    ...
    stg.Acquired,
    stg.IsAppraisalAppt
INTO #StageTSDDept_MTD_SourceData
FROM ETL_Staging.dbo.StageTrafficSummaryDailyDepartment_MTD stg WITH (NOLOCK)
...
WHERE stg.EntityKey IS NOT NULL
...
-- 2. Index temp table
------------------------------------------------------------------
CREATE CLUSTERED INDEX IX_SourceData_Key ON #StageTSDDept_MTD_SourceData
(
    EntityKey,
    FiscalMonthDateKey,
    szNewUsed,
...
BEGIN
    ...
    INTO #Batch
    FROM #StageTSDDept_MTD_SourceData
    ORDER BY
        EntityKey,
        FiscalMonthDateKey,
        szNewUsed,
    ...
    ------------------------------------------------------------------
    DELETE S
    FROM #StageTSDDept_MTD_SourceData S
    INNER JOIN #Batch B
