---
name: uspScoresFactOpportunityLoad_withTemp
database: Sonic_DW
type: procedure
schema: dbo
owner: Data Team
...
---- =============================================
CREATE PROCEDURE [dbo].[uspScoresFactOpportunityLoad_withTemp] (
...
SET @now = getdate();
	...
	SELECT ISNULL(DE.EntityKey, -1) AS [EntityKey]
			...
			,stg.sa_dmsstatus
			,SUM(CASE
					WHEN stg.statuscode in (
										100000044,
										100000051,
										100000052)
										and stg.sa_opportunitystate = 100000002
						THEN 1
					ELSE 0
					END) AS [SoldCount]
					----- removed 100000043,100000024,100000025  ----  Doug/Raj 03/14/2018
			--,SUM(CASE
...
--************************************************************	MIGRATE ELEAD OPPORTUNITIES TO SCORES AS NEEDED ***************************************
	...
	SELECT A.EntityKey
		...
		,ISNULL(CASE
				WHEN A.statuscode in (100000044,
									  100000051,
									  100000052)
				THEN CONVERT(INT, (CONVERT(VARCHAR(10), A.sa_closedate, 112)), 112)
				ELSE NULL
				END, 19000101) AS [DateSoldKey]
				----- removed 100000043,100000024,100000025,100000027  ----  Doug/Raj 03/14/2018
