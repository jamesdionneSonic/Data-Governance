# PL_Inventory_Equipment_Options_Getdata

Generated: 2026-06-29T10:58:59.576Z
...
PL_Inventory_Equipment_Options_Getdata is an ADF child pipeline in adf-pricefx-d1. If PL_Inventory_Equipment_Options_Getdata fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the parent pipeline that called PL_Inventory_Equipment_Options_Getdata; do not start the child with blank operational parameters.
...
| Platform              | ADF                                                                                                                                                                                                   |
| Asset type            | Pipeline                                                                                                                                                                                              |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-PriceFx-D1/pipelines/PL_Inventory_Equipment_Options_Getdata` |
| Support role          | child pipeline                                                                                                                                                                                        |
| Business process      | adf-pricefx-d1 pipeline execution                                                                                                                                                                     |
| Primary source        | DS_SQL_EPPricing_New, PriceFx_DB                                                                                                                                                                      |
| Primary target/output | DS_pfx_ieo_eppricing, DS_pfx_inv_equipmentOptions                                                                                                                                                     |
...
1. Check the parent pipeline that called PL_Inventory_Equipment_Options_Getdata; do not start the child with blank operational parameters.
...
3. Confirm source datasets are available: DS_SQL_EPPricing_New, PriceFx_DB.
4. Confirm target datasets or child pipelines completed: DS_pfx_ieo_eppricing, DS_pfx_inv_equipmentOptions.
...
| Source datasets   | DS_SQL_EPPricing_New, PriceFx_DB                                                                                                                                                                                       |
| Target datasets   | DS_pfx_ieo_eppricing, DS_pfx_inv_equipmentOptions                                                                                                                                                                      |
...
| ieo_eppricing                            | Copy                     | DS_SQL_EPPricing_New     | DS_pfx_ieo_eppricing        |
| Inv EO Data                              | Copy                     | PriceFx_DB               | DS_pfx_inv_equipmentOptions |
...
| Update delimiters                        | Script                   | not surfaced in metadata | not surfaced in metadata    |
| update failure information eppricing     | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata    |
| update failure information truncate tmps | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata    |
| update failure information vin lookup    | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata    |
| update failure information tmp_vmart     | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata    |
| update failure information delimiters    | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata    |
