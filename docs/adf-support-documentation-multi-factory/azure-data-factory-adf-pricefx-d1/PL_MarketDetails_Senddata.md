# PL_MarketDetails_Senddata

Generated: 2026-06-29T10:58:59.582Z
...
PL_MarketDetails_Senddata is an ADF child pipeline in adf-pricefx-d1. If PL_MarketDetails_Senddata fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the parent pipeline that called PL_MarketDetails_Senddata; do not start the child with blank operational parameters.
...
| Platform              | ADF                                                                                                                                                                                      |
| Asset type            | Pipeline                                                                                                                                                                                 |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-PriceFx-D1/pipelines/PL_MarketDetails_Senddata` |
| Support role          | child pipeline                                                                                                                                                                           |
| Business process      | adf-pricefx-d1 pipeline execution                                                                                                                                                        |
| Primary source        | ds_pricefx_marketdetails                                                                                                                                                                 |
...
This pipeline supports the adf-pricefx-d1 ADF process. Its available metadata shows 10 activity step(s), 0 child pipeline call(s), 1 source dataset reference(s), and 1 target dataset reference(s).

## Support Checks

1. Check the parent pipeline that called PL_MarketDetails_Senddata; do not start the child with blank operational parameters.
2. Confirm parameters are supplied by the parent, trigger, or documented default path: OpProcessExecutionID, OpBatchID, OpProcessExecutionStatusID, UserName, StartDate, EndDate, IsHistoricalDataLoad.
3. Confirm source datasets are available: ds_pricefx_marketdetails.
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Parent pipelines  | PL_PriceFx_MarketDetails_Master                                                                                                                                                 |
| Child pipelines   | none surfaced                                                                                                                                                                   |
| Source datasets   | ds_pricefx_marketdetails                                                                                                                                                        |
...
| cp_sftp_listingsmaster                       | Copy                     | ds_pricefx_marketdetails | DS_SFTP_US_EAST_1        |
| cp_sftp_listingsinventory                    | Copy                     | ds_pricefx_marketdetails | DS_SFTP_US_EAST_1        |
| update listing master failure information    | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
| update listing inventory failure information | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
