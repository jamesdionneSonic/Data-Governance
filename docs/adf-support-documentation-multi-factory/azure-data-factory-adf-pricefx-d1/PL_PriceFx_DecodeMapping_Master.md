# PL_PriceFx_DecodeMapping_Master

Generated: 2026-06-29T10:58:59.590Z
...
PL_PriceFx_DecodeMapping_Master is an ADF orchestrator in adf-pricefx-d1. If PL_PriceFx_DecodeMapping_Master fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the latest PL_PriceFx_DecodeMapping_Master parent run and identify the first failed child activity.
...
| Platform              | ADF                                                                                                                                                                                            |
| Asset type            | Pipeline                                                                                                                                                                                       |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-PriceFx-D1/pipelines/PL_PriceFx_DecodeMapping_Master` |
| Support role          | orchestrator                                                                                                                                                                                   |
| Business process      | adf-pricefx-d1 pipeline execution                                                                                                                                                              |
| Primary source        | not surfaced in metadata                                                                                                                                                                       |
| Primary target/output | PL_DecodeMapping_Getdata, PL_DecodeMapping_Senddata                                                                                                                                            |
| Schedule or trigger   | Tr_DecodeMapping                                                                                                                                                                               |
...
1. Check the latest PL_PriceFx_DecodeMapping_Master parent run and identify the first failed child activity.
...
4. Confirm target datasets or child pipelines completed: PL_DecodeMapping_Getdata, PL_DecodeMapping_Senddata.
...
| Child pipelines   | PL_DecodeMapping_Getdata, PL_DecodeMapping_Senddata                                                                                                                               |
...
| Execute DecodeMapping Getdata      | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
| Execute DecodeMapping Senddata     | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
| Set DecodeMapping Getdata Failure  | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
| Set DecodeMapping Senddata Failure | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
