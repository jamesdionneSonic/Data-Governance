# PL_MarketDetails_Getdata

Generated: 2026-06-29T10:58:59.581Z
...
PL_MarketDetails_Getdata is an ADF child pipeline in adf-pricefx-d1. If PL_MarketDetails_Getdata fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the parent pipeline that called PL_MarketDetails_Getdata; do not start the child with blank operational parameters.
...
| Platform              | ADF                                                                                                                                                                                     |
| Asset type            | Pipeline                                                                                                                                                                                |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-PriceFx-D1/pipelines/PL_MarketDetails_Getdata` |
...
1. Check the parent pipeline that called PL_MarketDetails_Getdata; do not start the child with blank operational parameters.
2. Confirm parameters are supplied by the parent, trigger, or documented default path: UserName, OpProcessExecutionID, OpBatchID, OpProcessExecutionStatusID, datadate, monthsBack, monthsForward.
...
| StartDate                         | SetVariable              | not surfaced in metadata | not surfaced in metadata |
| EndDate                           | SetVariable              | not surfaced in metadata | not surfaced in metadata |
| ReturnValues                      | SetVariable              | not surfaced in metadata | not surfaced in metadata |
