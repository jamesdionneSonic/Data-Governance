# PL_SalesTransactions_Senddata

Generated: 2026-06-29T10:58:59.606Z
...
PL_SalesTransactions_Senddata is an ADF child pipeline in adf-pricefx-d1. If PL_SalesTransactions_Senddata fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the parent pipeline that called PL_SalesTransactions_Senddata; do not start the child with blank operational parameters.
...
| Platform              | ADF                                                                                                                                                                                          |
| Asset type            | Pipeline                                                                                                                                                                                     |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-PriceFx-D1/pipelines/PL_SalesTransactions_Senddata` |
...
1. Check the parent pipeline that called PL_SalesTransactions_Senddata; do not start the child with blank operational parameters.
2. Confirm parameters are supplied by the parent, trigger, or documented default path: OpProcessExecutionID, OpBatchID, OpProcessExecutionStatusID, UserName, StartDate, EndDate.
...
| LKP_GetStartDateAndEndDate    | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| For Each Start and End Date   | ForEach                  | not surfaced in metadata | not surfaced in metadata |
