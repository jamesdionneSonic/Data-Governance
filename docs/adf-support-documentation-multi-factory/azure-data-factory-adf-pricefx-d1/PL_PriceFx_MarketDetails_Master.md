# PL_PriceFx_MarketDetails_Master

Generated: 2026-06-29T10:58:59.601Z
...
PL_PriceFx_MarketDetails_Master is an ADF orchestrator in adf-pricefx-d1. If PL_PriceFx_MarketDetails_Master fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the latest PL_PriceFx_MarketDetails_Master parent run and identify the first failed child activity.
...
| Platform              | ADF                                                                                                                                                                                            |
| Asset type            | Pipeline                                                                                                                                                                                       |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-PriceFx-D1/pipelines/PL_PriceFx_MarketDetails_Master` |
| Support role          | orchestrator                                                                                                                                                                                   |
| Business process      | adf-pricefx-d1 pipeline execution                                                                                                                                                              |
| Primary source        | not surfaced in metadata                                                                                                                                                                       |
| Primary target/output | PL_MarketDetails_Getdata, PL_MarketDetails_Senddata                                                                                                                                            |
| Schedule or trigger   | Tr_MarketDetails_Daily                                                                                                                                                                         |
...
1. Check the latest PL_PriceFx_MarketDetails_Master parent run and identify the first failed child activity.
...
4. Confirm target datasets or child pipelines completed: PL_MarketDetails_Getdata, PL_MarketDetails_Senddata.
...
| Child pipelines   | PL_MarketDetails_Getdata, PL_MarketDetails_Senddata                                                                                                                               |
...
| Execute MarketDetails Getdata      | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
| Execute MarketDetails Senddata     | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
| Set MarketDetails Getdata Failure  | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
| Set MarketDetails Senddata Failure | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
