# Store_Details_PriceFxDB

Generated: 2026-06-29T10:58:59.644Z
...
Store_Details_PriceFxDB is an ADF child pipeline in adf-pricefx-d1. If Store_Details_PriceFxDB fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the parent pipeline that called Store_Details_PriceFxDB; do not start the child with blank operational parameters.
...
| Platform              | ADF                                                                                                                                                                                    |
| Asset type            | Pipeline                                                                                                                                                                               |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-PriceFx-D1/pipelines/Store_Details_PriceFxDB` |
| Support role          | child pipeline                                                                                                                                                                         |
| Business process      | adf-pricefx-d1 pipeline execution                                                                                                                                                      |
| Primary source        | EPSIMS, RECON, DIMSQL, PriceFx_DB                                                                                                                                                      |
| Primary target/output | EPSIMS_PriceFxDB, RECON_PriceFxDB, DIMSQL_PriceFxDB, StoreDetailsPriceFx                                                                                                               |
...
This pipeline supports the adf-pricefx-d1 ADF process. Its available metadata shows 19 activity step(s), 0 child pipeline call(s), 4 source dataset reference(s), and 4 target dataset reference(s).

## Support Checks

1. Check the parent pipeline that called Store_Details_PriceFxDB; do not start the child with blank operational parameters.
...
3. Confirm source datasets are available: EPSIMS, RECON, DIMSQL, PriceFx_DB.
4. Confirm target datasets or child pipelines completed: EPSIMS_PriceFxDB, RECON_PriceFxDB, DIMSQL_PriceFxDB, StoreDetailsPriceFx.
...
| Source datasets   | EPSIMS, RECON, DIMSQL, PriceFx_DB                                                                                                                                                                                      |
| Target datasets   | EPSIMS_PriceFxDB, RECON_PriceFxDB, DIMSQL_PriceFxDB, StoreDetailsPriceFx                                                                                                                                               |
...
| --------------------------------------- | ------------------------ | ------------------------ | ------------------------ |
| Epsims_sql                              | Copy                     | EPSIMS                   | EPSIMS_PriceFxDB         |
| Recon_sql                               | Copy                     | RECON                    | RECON_PriceFxDB          |
| Dimsql                                  | Copy                     | DIMSQL                   | DIMSQL_PriceFxDB         |
| ToPriceFxDB                             | Copy                     | PriceFx_DB               | StoreDetailsPriceFx      |
| Truncate Tmp tables data                | Script                   | not surfaced in metadata | not surfaced in metadata |
| Get Child Parameters                    | Lookup                   | not surfaced in metadata | not surfaced in metadata |
...
| update failure information dim          | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
| update failure information recon        | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
