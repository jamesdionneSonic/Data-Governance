...
Generated: 2026-06-29T10:58:58.499Z
...
PL_LoadAQ_Full is an ADF orchestrator in adf-ganalytics-d1. If PL_LoadAQ_Full fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the latest PL_LoadAQ_Full parent run and identify the first failed child activity.
...
| Platform              | ADF                                                                                                                                                                              |
| Asset type            | Pipeline                                                                                                                                                                         |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-GAnalytics-D1/pipelines/PL_LoadAQ_Full` |
| Support role          | orchestrator                                                                                                                                                                     |
| Business process      | adf-ganalytics-d1 pipeline execution                                                                                                                                             |
| Primary source        | not surfaced in metadata                                                                                                                                                         |
| Primary target/output | PL_LoadAQ_CheckParentProcesses, PL_LoadAQ_ViewsToTables                                                                                                                          |
...
This pipeline supports the adf-ganalytics-d1 ADF process. Its available metadata shows 2 activity step(s), 2 child pipeline call(s), 0 source dataset reference(s), and 0 target dataset reference(s).

## Support Checks

1. Check the latest PL_LoadAQ_Full parent run and identify the first failed child activity.
...
4. Confirm target datasets or child pipelines completed: PL_LoadAQ_CheckParentProcesses, PL_LoadAQ_ViewsToTables.
...
| Child pipelines   | PL_LoadAQ_CheckParentProcesses, PL_LoadAQ_ViewsToTables |
...
| ------------------------------- | --------------- | ------------------------ | ------------------------ |
| ACT_EXECPL_CheckParentProcesses | ExecutePipeline | not surfaced in metadata | not surfaced in metadata |
| ACT_EXECPL_LoadViewsToTables    | ExecutePipeline | not surfaced in metadata | not surfaced in metadata |
