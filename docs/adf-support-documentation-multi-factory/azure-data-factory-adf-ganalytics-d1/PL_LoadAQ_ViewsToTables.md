# PL_LoadAQ_ViewsToTables

Generated: 2026-06-29T10:58:58.503Z
...
PL_LoadAQ_ViewsToTables is an ADF child pipeline in adf-ganalytics-d1. If PL_LoadAQ_ViewsToTables fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the parent pipeline that called PL_LoadAQ_ViewsToTables; do not start the child with blank operational parameters.
...
| Platform              | ADF                                                                                                                                                                                       |
| Asset type            | Pipeline                                                                                                                                                                                  |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-GAnalytics-D1/pipelines/PL_LoadAQ_ViewsToTables` |
...
1. Check the parent pipeline that called PL_LoadAQ_ViewsToTables; do not start the child with blank operational parameters.
2. Confirm parameters are supplied by the parent, trigger, or documented default path: UserName, TriggerScheduledTime, FirstRun.
...
| ----------------- | ------------------------ |
| Parent pipelines  | PL_LoadAQ_Full           |
...
| --------------------------- | ----------- | ------------------------ | ------------------------ |
| ACT_LKP_GetPLDSValues       | Lookup      | not surfaced in metadata | not surfaced in metadata |
| ForEachTable                | ForEach     | not surfaced in metadata | not surfaced in metadata |
| ACT_IF_GALoaded             | IfCondition | not surfaced in metadata | not surfaced in metadata |
| ACT_LKP_CheckGALoad         | Lookup      | not surfaced in metadata | not surfaced in metadata |
| ACT_SCR_UpdForAll           | Script      | not surfaced in metadata | not surfaced in metadata |
...
| ACT_LKP_GetViewsLoadedCount | Lookup      | not surfaced in metadata | not surfaced in metadata |
| ACT_IF_ViewsLoadedCount0    | IfCondition | not surfaced in metadata | not surfaced in metadata |
