# PL_LoadAQ_RefreshMSTRCubes

Generated: 2026-06-29T10:58:58.501Z
...
PL_LoadAQ_RefreshMSTRCubes is an ADF standalone or utility pipeline in adf-ganalytics-d1. If PL_LoadAQ_RefreshMSTRCubes fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the latest ADF run for PL_LoadAQ_RefreshMSTRCubes and confirm source and target datasets are available.
...
| Platform              | ADF                                                                                                                                                                                          |
| Asset type            | Pipeline                                                                                                                                                                                     |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-GAnalytics-D1/pipelines/PL_LoadAQ_RefreshMSTRCubes` |
...
This pipeline supports the adf-ganalytics-d1 ADF process. Its available metadata shows 7 activity step(s), 0 child pipeline call(s), 0 source dataset reference(s), and 0 target dataset reference(s).

## Support Checks

1. Check the latest ADF run for PL_LoadAQ_RefreshMSTRCubes and confirm source and target datasets are available.
2. Confirm parameters are supplied by the parent, trigger, or documented default path: UserName, TriggerScheduledTime, FirstRun, LastRun.
...
| ACT_LKP_GetTriggers       | Lookup      | not surfaced in metadata | not surfaced in metadata |
| ACT_FE_MSTRCubes          | ForEach     | not surfaced in metadata | not surfaced in metadata |
| ACT_WEB_GetAuthToken      | WebActivity | not surfaced in metadata | not surfaced in metadata |
| ACT_IF_CubesRefreshCount0 | IfCondition | not surfaced in metadata | not surfaced in metadata |
| ACT_LKP_CubesRefreshCount | Lookup      | not surfaced in metadata | not surfaced in metadata |
