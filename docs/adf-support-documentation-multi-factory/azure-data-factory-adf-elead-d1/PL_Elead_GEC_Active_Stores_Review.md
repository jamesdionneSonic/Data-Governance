# PL_Elead_GEC_Active_Stores_Review

Generated: 2026-06-29T10:58:58.710Z
...
PL_Elead_GEC_Active_Stores_Review is an ADF standalone or utility pipeline in adf-elead-d1. If PL_Elead_GEC_Active_Stores_Review fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the latest ADF run for PL_Elead_GEC_Active_Stores_Review and confirm source and target datasets are available.
...
| Platform              | ADF                                                                                                                                                                                            |
| Asset type            | Pipeline                                                                                                                                                                                       |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-eLead-D1/pipelines/PL_Elead_GEC_Active_Stores_Review` |
...
This pipeline supports the adf-elead-d1 ADF process. Its available metadata shows 12 activity step(s), 0 child pipeline call(s), 0 source dataset reference(s), and 0 target dataset reference(s).

## Support Checks

1. Check the latest ADF run for PL_Elead_GEC_Active_Stores_Review and confirm source and target datasets are available.
2. Confirm parameters are supplied by the parent, trigger, or documented default path: UserName.
...
| ------------------------------------- | ------------------------ | ------------------------ | ------------------------ |
| CheckTables                           | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| GetMissingActiveGECStores             | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| GetNewStores                          | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| FEMissingGECStoreResult               | ForEach                  | not surfaced in metadata | not surfaced in metadata |
| Set MsgMissingStores                  | SetVariable              | not surfaced in metadata | not surfaced in metadata |
| FENewStores                           | ForEach                  | not surfaced in metadata | not surfaced in metadata |
| Set MsgNewStore                       | SetVariable              | not surfaced in metadata | not surfaced in metadata |
| Generate Store Level Review Report    | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
| PackageLoadDateSync                   | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| Update_PackageLoadDateSync            | Lookup                   | not surfaced in metadata | not surfaced in metadata |
