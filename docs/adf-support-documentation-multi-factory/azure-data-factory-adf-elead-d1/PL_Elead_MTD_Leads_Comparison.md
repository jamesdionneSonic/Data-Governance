# PL_Elead_MTD_Leads_Comparison

Generated: 2026-06-29T10:58:58.728Z
...
PL_Elead_MTD_Leads_Comparison is an ADF standalone or utility pipeline in adf-elead-d1. If PL_Elead_MTD_Leads_Comparison fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the latest ADF run for PL_Elead_MTD_Leads_Comparison and confirm source and target datasets are available.
...
| Platform              | ADF                                                                                                                                                                                        |
| Asset type            | Pipeline                                                                                                                                                                                   |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-eLead-D1/pipelines/PL_Elead_MTD_Leads_Comparison` |
...
This pipeline supports the adf-elead-d1 ADF process. Its available metadata shows 9 activity step(s), 0 child pipeline call(s), 0 source dataset reference(s), and 0 target dataset reference(s).

## Support Checks

1. Check the latest ADF run for PL_Elead_MTD_Leads_Comparison and confirm source and target datasets are available.
...
| GetListOfTables                       | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| GetLeadsComparison                    | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| ForEachResultItem                     | ForEach                  | not surfaced in metadata | not surfaced in metadata |
| Set Message                           | SetVariable              | not surfaced in metadata | not surfaced in metadata |
| Generate MTD Leads Report             | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
