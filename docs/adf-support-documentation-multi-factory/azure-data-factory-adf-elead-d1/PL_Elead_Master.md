...
Generated: 2026-06-29T10:58:58.720Z
...
PL_Elead_Master is an ADF orchestrator in adf-elead-d1. If PL_Elead_Master fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the latest PL_Elead_Master parent run and identify the first failed child activity.
...
| Platform              | ADF                                                                                                                                                                          |
| Asset type            | Pipeline                                                                                                                                                                     |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-eLead-D1/pipelines/PL_Elead_Master` |
| Support role          | orchestrator                                                                                                                                                                 |
| Business process      | adf-elead-d1 pipeline execution                                                                                                                                              |
| Primary source        | not surfaced in metadata                                                                                                                                                     |
| Primary target/output | PL_Elead_Incremental_Load                                                                                                                                                    |
| Schedule or trigger   | Elead_Daily                                                                                                                                                                  |
...
This pipeline supports the adf-elead-d1 ADF process. Its available metadata shows 11 activity step(s), 1 child pipeline call(s), 0 source dataset reference(s), and 0 target dataset reference(s).

## Support Checks

1. Check the latest PL_Elead_Master parent run and identify the first failed child activity.
...
4. Confirm target datasets or child pipelines completed: PL_Elead_Incremental_Load.
...
| Child pipelines   | PL_Elead_Incremental_Load |
...
| PL_Elead_Incremental_Load             | ExecutePipeline          | not surfaced in metadata | not surfaced in metadata |
| GetErrorFileLists                     | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| EleadFailureMailAlert                 | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
