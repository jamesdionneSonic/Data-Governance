...
Generated: 2026-06-29T10:58:58.336Z
...
PL_MCI_EPCallSource is an ADF child pipeline in adf-mci-d1. If PL_MCI_EPCallSource fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the parent pipeline that called PL_MCI_EPCallSource; do not start the child with blank operational parameters.
...
| Platform              | ADF                                                                                                                                                                            |
| Asset type            | Pipeline                                                                                                                                                                       |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-MCI-D1/pipelines/PL_MCI_EPCallSource` |
| Support role          | child pipeline                                                                                                                                                                 |
| Business process      | adf-mci-d1 pipeline execution                                                                                                                                                  |
| Primary source        | DS_SQL_EPCallSource                                                                                                                                                            |
| Primary target/output | DS_SFTP_EPCallsource                                                                                                                                                           |
...
1. Check the parent pipeline that called PL_MCI_EPCallSource; do not start the child with blank operational parameters.
2. Confirm parameters are supplied by the parent, trigger, or documented default path: ETLExecID, UserName.
3. Confirm source datasets are available: DS_SQL_EPCallSource.
4. Confirm target datasets or child pipelines completed: DS_SFTP_EPCallsource.
...
| ----------------- | ------------------------------------- |
| Parent pipelines  | PL_MCI_EPCallSource_Incremental       |
| Child pipelines   | none surfaced                         |
| Source datasets   | DS_SQL_EPCallSource                   |
| Target datasets   | DS_SFTP_EPCallsource                  |
...
| ------------------------------------- | ------------------------ | ------------------------ | ------------------------ |
| Load EPCallSource File \_ SQL To SFTP | Copy                     | DS_SQL_EPCallSource      | DS_SFTP_EPCallsource     |
| SET EPCallSource Count                | SetVariable              | not surfaced in metadata | not surfaced in metadata |
