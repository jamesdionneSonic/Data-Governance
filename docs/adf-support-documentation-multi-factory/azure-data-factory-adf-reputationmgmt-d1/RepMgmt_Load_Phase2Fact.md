# RepMgmt_Load_Phase2Fact

Generated: 2026-06-29T10:58:57.796Z
...
RepMgmt_Load_Phase2Fact is an ADF standalone or utility pipeline in adf-reputationmgmt-d1. If RepMgmt_Load_Phase2Fact fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the latest ADF run for RepMgmt_Load_Phase2Fact and confirm source and target datasets are available.
...
| Platform              | ADF                                                                                                                                                                                           |
| Asset type            | Pipeline                                                                                                                                                                                      |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/rg-data-warehouse-prod/providers/Microsoft.DataFactory/factories/adf-ReputationMgmt-D1/pipelines/RepMgmt_Load_Phase2Fact` |
...
This pipeline supports the adf-reputationmgmt-d1 ADF process. Its available metadata shows 5 activity step(s), 0 child pipeline call(s), 0 source dataset reference(s), and 0 target dataset reference(s).

## Support Checks

1. Check the latest ADF run for RepMgmt_Load_Phase2Fact and confirm source and target datasets are available.
...
| -------------------------------- | ------------------------ | ------------------------ | ------------------------ |
| LoadFactRMReviewMetrics          | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| InsertAuditDetailsPhase2         | SqlServerStoredProcedure | not surfaced in metadata | not surfaced in metadata |
| LoadFactRMReviewMonthly          | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| UpdatePackageLoadDateSyncMonthly | Lookup                   | not surfaced in metadata | not surfaced in metadata |
| UpdatePackageLoadDateSyncMetrics | Lookup                   | not surfaced in metadata | not surfaced in metadata |
