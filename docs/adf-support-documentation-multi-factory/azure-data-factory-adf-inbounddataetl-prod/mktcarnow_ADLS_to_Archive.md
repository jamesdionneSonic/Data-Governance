# mktcarnow_ADLS_to_Archive

Generated: 2026-06-29T10:58:59.107Z
...
mktcarnow_ADLS_to_Archive is an ADF standalone or utility pipeline in adf-inbounddataetl-prod. If mktcarnow_ADLS_to_Archive fails or becomes stale, dependent ADF loads, files, datasets, or downstream reporting may be incomplete until the failed run is diagnosed and rerun through the approved parent path. Start troubleshooting by checking the latest ADF run for mktcarnow_ADLS_to_Archive and confirm source and target datasets are available.
...
| Platform              | ADF                                                                                                                                                                                                    |
| Asset type            | Pipeline                                                                                                                                                                                               |
| Native path           | `/subscriptions/bee9b611-da99-4cfc-9fb7-50f1359e5ca2/resourceGroups/RG-InboundDataServices-Prod/providers/Microsoft.DataFactory/factories/ADF-InboundDataETL-Prod/pipelines/mktcarnow_ADLS_to_Archive` |
...
This pipeline supports the adf-inbounddataetl-prod ADF process. Its available metadata shows 2 activity step(s), 0 child pipeline call(s), 0 source dataset reference(s), and 0 target dataset reference(s).

## Support Checks

1. Check the latest ADF run for mktcarnow_ADLS_to_Archive and confirm source and target datasets are available.
...
| -------------------------------------- | ----------- | ------------------------ | ------------------------ |
| Get List of Files from ADLS            | GetMetadata | not surfaced in metadata | not surfaced in metadata |
| Iterate thru the ADLS Carnow Container | ForEach     | not surfaced in metadata | not surfaced in metadata |
